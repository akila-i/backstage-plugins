...
describe('EventDeltaApplier.handleEvent', () => {
  ...
  // ---- Wirelogs cascade on DataPlane Cilium change ------------------
  // A DataPlane gaining Cilium must flip the Wirelogs tab on for the
  // components of every project whose environments it backs, without
  // waiting for the periodic full sync. The cascade walks the catalog
  // relation graph: Environment(data-plane-ref) -> deployedBy pipeline
  // -> pipelineUsedBy project -> hasPart components.
  it('refreshes affected components when a DataPlane gains Cilium', async () => {
    // Catalog graph: env `dev` (backed by dp-dev) -> pipeline `pipe`
    // -> project `proj` -> component `api` (currently tab-hidden).
    const env = {
      kind: 'Environment',
      metadata: {
        namespace: 'test-ns',
        name: 'dev',
        annotations: { 'openchoreo.io/data-plane-ref': 'dp-dev' },
      },
      relations: [
        { type: 'deployedBy', targetRef: 'deploymentpipeline:test-ns/pipe' },
      ],
    };
    const byRef: Record<string, any> = {
      'deploymentpipeline:test-ns/pipe': {
        kind: 'DeploymentPipeline',
        metadata: { namespace: 'test-ns', name: 'pipe' },
        relations: [
          { type: 'pipelineUsedBy', targetRef: 'system:test-ns/proj' },
        ],
      },
      'system:test-ns/proj': {
        kind: 'System',
        metadata: { namespace: 'test-ns', name: 'proj' },
        relations: [{ type: 'hasPart', targetRef: 'component:test-ns/api' }],
      },
      'component:test-ns/api': {
        kind: 'Component',
        metadata: {
          namespace: 'test-ns',
          name: 'api',
          annotations: { 'openchoreo.io/wirelogs-enabled': 'false' },
        },
      },
    };
    const catalogService = {
      getEntities: jest.fn(async () => ({ items: [env] })),
      getEntitiesByRefs: jest.fn(async ({ entityRefs }: any) => ({
        items: entityRefs.map((r: string) => byRef[r]),
      })),
    } as any;

    // API client: dp-dev now runs Cilium; project/pipeline/env resolve so
    // computeWirelogsEnabled returns true; component refetch + empty workload.
    mockGET.mockImplementation((path: string) => {
      if (path.endsWith('/dataplanes/{dpName}')) {
        return Promise.resolve(
          okData({
            metadata: {
              name: 'dp-dev',
              annotations: { 'openchoreo.dev/networkpolicyprovider': 'cilium' },
            },
          }),
        );
      }
      if (path.endsWith('/projects/{projectName}')) {
        return Promise.resolve(
          okData({
            metadata: { name: 'proj' },
            spec: { deploymentPipelineRef: { name: 'pipe' } },
          }),
        );
      }
      if (path.endsWith('/deploymentpipelines/{deploymentPipelineName}')) {
        return Promise.resolve(
          okData({
            metadata: { name: 'pipe' },
            spec: {
      ...
      if (path.endsWith('/environments/{envName}')) {
        return Promise.resolve(
          okData({
            metadata: { name: 'dev' },
            spec: { dataPlaneRef: { kind: 'DataPlane', name: 'dp-dev' } },
          }),
        );
      }
      if (path.endsWith('/components/{componentName}')) {
        return Promise.resolve(
          okData({
            metadata: { name: 'api' },
            spec: {
    ...
    const applier = newApplierWithCatalog(catalogService);
    await applier.handleEvent('DataPlane', 'dp-dev', 'test-ns', 'updated');

    // Environments queried by the data-plane-ref annotation, scoped to ns.
    expect(catalogService.getEntities).toHaveBeenCalledTimes(1);
    expect(catalogService.getEntities.mock.calls[0][0].filter).toMatchObject({
      kind: 'Environment',
      'metadata.annotations.openchoreo.io/data-plane-ref': 'dp-dev',
      'metadata.namespace': 'test-ns',
    });

    // The component was re-emitted with the tab now enabled.
    const added = applyMutation.mock.calls.flatMap((c: any) =>
      (c[0].added ?? []).map((d: any) => d.entity),
    );
    const component = added.find(
      (e: any) => e?.kind === 'Component' && e.metadata.name === 'api',
    );
    expect(component).toBeDefined();
    expect(
      component.metadata.annotations['openchoreo.io/wirelogs-enabled'],
    ).toBe('true');
  });

  it('does not refresh components when the DataPlane value is unchanged', async () => {
    // Same graph, but the component is already tab-enabled and the DataPlane
    // still runs Cilium — the cascade must skip the redundant re-emit.
    const env = {
      kind: 'Environment',
      metadata: {
        namespace: 'test-ns',
    ...
    const byRef: Record<string, any> = {
      ...
      'component:test-ns/api': {
        ...
        metadata: {
          namespace: 'test-ns',
          name: 'api',
          annotations: { 'openchoreo.io/wirelogs-enabled': 'true' },
        },
      },
    };
    const catalogService = {
    ...
    // Only the DataPlane entity itself was upserted; no component re-emit.
    const added = applyMutation.mock.calls.flatMap((c: any) =>
      (c[0].added ?? []).map((d: any) => d.entity),
    );
    expect(added.some((e: any) => e?.kind === 'Component')).toBe(false);
  });
});
