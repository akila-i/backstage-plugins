...
export const NotificationChannelFormWithYamlExtension = ({
  ...
  const data: NotificationChannelWorkingData = useMemo(
    () => ({
      ...DEFAULT_FORM_DATA,
      ...formData,
      emailConfig: { ...DEFAULT_FORM_DATA.emailConfig, ...formData?.emailConfig },
      webhookConfig: {
  ...
  // otherwise the review step (and the created resource) would carry stale
  // values left over from the other channel type.
  ...
  const updateWebhookField = useCallback(
    (
      field: keyof NotificationChannelWebhookConfigFormData,
      value: unknown,
    ) => {
      commit({
        ...data,
      ...
      {mode === 'form' ? (
        ...
        <div>
          <div className={classes.helpText}>
            <span>
              Edit the ObservabilityAlertsNotificationChannel CR YAML
              directly.
            </span>
          </div>
